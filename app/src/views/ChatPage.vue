<template>
  <div class="w-full h-full min-h-0 mx-auto overflow-x-hidden">
    ...
    >
      ...
      <!-- 登录提示 -->
      <!-- 未登录显示登录组件 -->
      <ChatAuth v-if="!hasToken()" @login-success="handleLoginSuccess" />
                  ...
                  >
                    ...
                    >
                      ...
                      <template v-else-if="isMe(m) && m.__status === 'failed'">
                        <i class="ri-refresh-line text-[9px] text-rose-500"></i>
                      </template>
                      <template v-else>
        ...
        <div class="relative">
          ...
          <div ref="composerInputRowRef" class="flex items-center gap-2 max-w-5xl mx-auto min-w-0">
            ...
            >
              ...
              <i v-else class="ri-send-plane-fill text-sm"></i>
            </button>
          </div>
        </div>
      </div>

      <Drawer v-model="showSettings" title="个人设置">
        <div class="mx-5 mb-4 px-4 py-2 rounded-lg theme-card-soft text-xs theme-text-secondary flex items-center gap-2">
          <i class="ri-information-line text-sm"></i>
          <span>填写 QQ 自动同步头像，可不填</span>
        </div>
        <div class="flex gap-4 px-5 pb-2">
          <div class="flex items-center justify-center w-1/2">
            <div
              class="w-20 h-20 rounded-full theme-card-soft flex items-center justify-center overflow-hidden shadow-inner relative"
            >
              <div
          ...
          <div class="w-1/2 space-y-4">
            ...
            <div class="space-y-1">
              <label class="text-[9px] font-bold uppercase theme-text-secondary ml-1">QQ 号</label>
              <div
                v-if="loadingUserSettings"
                class="h-9 w-full theme-card-soft animate-pulse rounded-xl"
              ></div>
        ...
        <div class="flex gap-3 px-5 mt-4 mb-4">
          <button
            @click="showSettings = false"
            :disabled="isSavingSettings"
            class="flex-1 py-2.5 rounded-xl text-xs font-bold active:scale-95 transition-all disabled:opacity-50 theme-card-soft theme-text-secondary"
          >
            取消
          </button>
          <button
            @click="applySettings"
            :disabled="isSavingSettings"
            class="flex-1 py-2.5 rounded-xl text-xs font-bold active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2 text-white" style="background: #2563eb"
          >
        ...
        >
          <div class="p-6">
            <div class="flex items-center gap-2 mb-4 theme-text-primary">
              <i class="ri-shield-line text-blue-500"></i>
              <h3 class="font-bold text-sm">隐私说明</h3>
            </div>
...
async function send() {
  ...
  try {
    ...
    // 发送
    const sendRes = await client.sendMessage(mergedParts, replyingTo.value?.id);

    // 乐观更新：立即将消息插入列表，Socket 事件到达时自动去重替换
    const sentMsg = sendRes?.message;
    if (sentMsg && sentMsg.id && !messages.value.some((m) => String(m.id) === String(sentMsg.id))) {
      messages.value.push({ ...sentMsg, user: sentMsg.user || user.value });
      markChatSeen(String(sentMsg.created_at || '').trim() || new Date().toISOString());
    }
...
.menu-pop-animation {
  animation: menu-pop 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}

.menu-pop-animation-up {
  animation: menu-pop-up 0.3s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
}
