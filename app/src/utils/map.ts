// 导入各地图的坐标数据（cuit_hkg、cuit_lqy等为具体地图标识）
...
const mapDataCollection: Record<string, string[]> = {
  ...
  ncwsxx,
  sctbc,
  default: cuit_hkg, // 默认使用cuit_hkg地图的坐标数据
};
...
// 注：randInt（随机整数生成）和randPos（随机点生成）函数已移除，
// 插值计算和时间控制逻辑现在统一在genTrackPoints函数内部处理
...
export function genTrackPoints(distance: number, mapChoice: string = "default"): string {
  ...
  const result: string[] = [];       // 存储最终轨迹点的数组
   ...
   */
  const metersToLngLat = (dxMeters: number, dyMeters: number, lat: number) => {
  ...
  while (generatedDistance < distance) {
    ...
    const n = Math.min(maxPointsPerSegment, Math.max(1, Math.ceil(segDist / targetSpacing)));
    ...
    const approxMetersPerDegLng = (Math.PI / 180) * 6378137 * Math.cos((lat * Math.PI) / 180) / (Math.PI / 180);
    const segVecMeters = [dx * approxMetersPerDegLng, dy * approxMetersPerDegLat]; // 米单位段向量
    ...
    let prevSegVecMeters: number[] = [0, 0]; // 前一段向量（默认初始为0向量）
    if (result.length >= 2) { // 至少有2个轨迹点时，才能计算前一段向量
      ...
      // 计算前一段的米单位向量
      prevSegVecMeters = [(lx - sx) * approxMetersPerDegLng, (ly - sy) * approxMetersPerDegLat];
    }
    ...
    for (let i = 1; i <= n; i++) {
      ...
      const lateral = (Math.random() - 0.5) * 2 * lateralMax * turnBoost * (Math.sin(Math.PI * t));
