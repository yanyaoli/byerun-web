<template>
  <div class="club-page min-h-full pb-4">
    <section class="theme-card rounded-xl p-4 relative z-10 overflow-visible">
      <div class="flex items-center gap-2 mb-3 bg-[var(--card-soft-bg)] rounded-lg p-1">
        <button
          v-for="tab in MAIN_TABS"
          :key="tab.key"
          type="button"
          :class="[
            'flex-1 h-8 rounded-lg text-xs font-medium transition-all text-center inline-flex items-center justify-center gap-1.5',
            activeMainTab === tab.key
              ? 'bg-[var(--card-bg)] shadow-sm theme-text-primary'
      ...
      <div v-if="activeMainTab === 'activities'" class="flex items-center gap-2 mb-3 bg-[var(--card-soft-bg)] rounded-lg p-1">
        <button
          v-for="tab in ACTIVITY_SUB_TABS"
          :key="tab.key"
          type="button"
          :class="[
            'flex-1 h-7 rounded-lg text-[11px] font-medium transition-all text-center inline-flex items-center justify-center gap-1',
      ...
      <div v-if="activeMainTab === 'history'" class="flex items-center gap-2 mb-3 bg-[var(--card-soft-bg)] rounded-lg p-1">
        <button
      ...
      <div class="flex items-start gap-2">
        ...
        >
          <button
            type="button"
            class="w-full h-9 px-3 rounded-xl theme-card-soft text-xs theme-text-primary inline-flex items-center justify-between relative z-10 gap-2"
            @click="showDateDropdown = !showDateDropdown"
          >
            <span class="flex items-center gap-1.5 truncate">
              <i class="ri-calendar-line text-sm theme-text-tertiary"></i>
          ...
          <Drawer v-model="showDateDropdown" title="选择日期">
            <div
        ...
        <button
          type="button"
          class="h-9 px-3 rounded-xl text-xs font-medium flex items-center gap-1.5 transition-colors"
          :class="showFilters ? 'bg-[var(--card-bg)] shadow-sm theme-text-primary' : 'theme-card-soft theme-text-primary'"
          @click="showFilters = !showFilters"
        >
          <i :class="showFilters ? 'ri-equalizer-fill' : 'ri-equalizer-line'" class="text-sm"></i>
          <span>筛选</span>
        </button>
      </div>
        ...
        <div v-show="showFilters" class="mt-3 space-y-2">
          <div class="flex items-center gap-2 overflow-x-auto pb-1">
            <button
              ...
              type="button"
              :class="[
                'shrink-0 h-8 px-3 rounded-full border text-xs transition-colors inline-flex items-center gap-1.5',
                selectedStatus === option.value
                  ? 'bg-[var(--card-bg)] shadow-sm theme-text-primary border-[var(--card-border)]'
                  : 'theme-card-soft theme-text-secondary',
              ]"
              @click="selectedStatus = option.value"
            >
              <i v-if="option.icon" :class="option.icon" class="text-[11px]"></i>
              {{ option.label }}
            </button>
          </div>
          ...
          >
            ...
            >
              <i class="ri-grid-fill text-[11px]"></i>
              {{ item.label }}
            </button>
          </div>
        </div>
    ...
    <section v-if="activeMainTab === 'history'" class="mt-3 flex gap-2">
      <div class="flex-1 rounded-xl px-3 py-2.5" style="background: rgba(8, 145, 178, 0.06)">
        <div class="text-[11px] theme-text-secondary">累计报名</div>
        <div class="mt-0.5 text-lg font-bold" style="color: #0891b2">{{ summary.joinNum }}</div>
      </div>
      <div class="flex-1 rounded-xl px-3 py-2.5" style="background: rgba(22, 163, 74, 0.06)">
        <div class="text-[11px] theme-text-secondary">有效签到</div>
        <div class="mt-0.5 text-lg font-bold" style="color: #16a34a">{{ summary.validNum }}</div>
      </div>
    </section>
    ...
    <section
      ...
      class="mt-3 theme-card rounded-xl p-4" style="border-color: var(--warning-border); background: var(--warning-bg)"
    >
      <div class="flex items-center justify-between gap-2">
        <h3 class="text-xs font-semibold" style="color: var(--warning-color)">已安排抢选任务</h3>
        <span class="text-[11px]" style="color: var(--warning-color); opacity: 0.7">{{ clubRushTasks.length }} 条</span>
      </div>
      ...
      <div class="mt-2 space-y-2">
        <div
          ...
          :key="`rush-${task.id}`"
          class="rounded-xl px-3 py-2" style="border: 1px solid var(--warning-border); background: var(--card-soft-bg)"
        >
          <div class="flex items-center justify-between gap-2">
            <span class="text-xs" style="color: var(--warning-color)">活动ID {{ task.activityId }}</span>
            <div class="flex items-center gap-2">
              <button
                v-if="task.canCancel"
                type="button"
                :disabled="isClubActionPending(task.cancelPendingKey)"
                class="h-6 px-2 rounded-md text-[11px] inline-flex items-center gap-1 theme-danger-bg text-white disabled:opacity-70 disabled:cursor-not-allowed"
          ...
          <div class="mt-1 text-[11px]" style="color: var(--warning-color); opacity: 0.75">
            执行时间：{{ task.executeAt }}
          </div>
          <div v-if="task.lastResult" class="mt-1 text-[11px]" style="color: var(--warning-color); opacity: 0.65">
            结果：{{ task.lastResult }}
          </div>
        </div>
      </div>
    ...
    <section class="mt-3 flex items-center justify-between text-xs theme-text-secondary px-1">
      <span class="flex items-center gap-1.5">
        <i class="ri-list-check text-[11px]"></i>
        {{ currentListTitle }}
      </span>
    ...
    <section class="mt-2 space-y-3">
      <template v-if="loading">
        <div
          v-for="idx in 4"
          :key="`skeleton-${idx}`"
          class="theme-card rounded-xl p-4 animate-pulse"
        >
          <div class="h-4 w-2/3 rounded" style="background: var(--card-soft-bg)"></div>
          <div class="mt-3 h-3 w-full rounded" style="background: var(--card-soft-bg)"></div>
          <div class="mt-2 h-3 w-4/5 rounded" style="background: var(--card-soft-bg)"></div>
        </div>
      </template>

      <div
        v-else-if="cards.length === 0"
        class="theme-card rounded-xl p-6 text-center text-sm theme-text-secondary flex flex-col items-center gap-3"
      >
        <div class="w-12 h-12 rounded-full bg-[var(--card-soft-bg)] flex items-center justify-center">
          <i class="ri-inbox-2-line text-xl theme-text-tertiary"></i>
        </div>
        {{ emptyMessage }}
      </div>

      <article v-for="card in cards" :key="card.key" class="theme-card rounded-xl p-4">
        ...
        <template v-else>
          <div class="flex items-start justify-between gap-2">
            <div class="min-w-0 flex-1">
              ...
              <div class="mt-1 flex items-center gap-2 text-xs theme-text-tertiary">
                <span v-if="card.subTitle" class="truncate">{{ card.subTitle }}</span>
                <span v-if="card.metaTertiaryText" class="truncate opacity-70">· {{ card.metaTertiaryText }}</span>
              </div>
            </div>
            <span :class="card.badgeClass">{{ card.badgeText }}</span>
          </div>
          ...
          <div class="mt-3 flex items-center justify-between gap-2">
            <div class="flex items-center gap-3 text-[11px] theme-text-tertiary">
              <span class="flex items-center gap-1">
              ...
              <span class="flex items-center gap-1">
                ...
                <span>{{ card.metaSecondaryText }}</span>
              </span>
            </div>
            <div class="flex items-center gap-2">
              <button
          ...
          <p v-if="card.showIntro" class="mt-3 text-xs theme-text-secondary leading-5">
            {{ card.introText }}
          </p>
        </template>
      </article>
      ...
      >
        <button
          type="button"
          class="w-full py-2.5 rounded-xl text-xs font-medium transition-all theme-text-secondary hover:bg-[var(--action-hover-bg)] active:scale-[0.98] inline-flex items-center justify-center gap-1.5 disabled:opacity-50"
...
const MAIN_TABS = [
  ...
  { key: 'history', label: '历史记录', icon: 'ri-history-line' },
];

const ACTIVITY_SUB_TABS = [
  { key: 'list', label: '活动列表', icon: 'ri-list-check' },
  { key: 'myTask', label: '我的任务', icon: 'ri-user-star-line' },
];

const HISTORY_SUB_TABS = [
  { key: 'record', label: '签到记录', icon: 'ri-file-list-3-line' },
  { key: 'semester', label: '学期记录', icon: 'ri-bar-chart-grouped-line' },
];

const STATUS_OPTIONS_MAP = {
  pending: [
    { value: 'all', label: '全部', icon: '' },
    { value: 'joined', label: '已报名', icon: 'ri-checkbox-circle-line' },
    { value: 'joinable', label: '可报名', icon: 'ri-add-circle-line' },
    { value: 'full', label: '报名已满', icon: 'ri-close-circle-line' },
    { value: 'blocked', label: '已报其他', icon: 'ri-forbid-2-line' },
    { value: 'cancelled', label: '已取消', icon: 'ri-close-line' },
  ],
  semester: [
    { value: 'all', label: '全部', icon: '' },
    { value: 'completed', label: '已完成', icon: 'ri-check-line' },
    { value: 'pending', label: '未完成', icon: 'ri-time-line' },
  ],
  history: [
    { value: 'all', label: '全部', icon: '' },
    { value: 'completed', label: '已完成', icon: 'ri-check-line' },
    { value: 'unsigned', label: '未签到', icon: 'ri-question-line' },
    { value: 'notSignOut', label: '未签退', icon: 'ri-logout-box-r-line' },
  ],
};
...
function resolveBadge(item) {
  ...
  if (activeMainTab.value === 'activities' && activeActivityTab.value === 'myTask') {
    switch (String(item.activityStatus)) {
      case '1':
        return createBadge('可报名', 'text-blue-600 bg-blue-500/10 border border-blue-500/20');
      case '2':
        return createBadge('进行中', 'text-blue-600 bg-blue-500/10 border border-blue-500/20');
      case '3':
...
function resolveClubAction(item) {
  ...
  if (optionStatus === '6') {
    return {
      ...
      pendingLabel: '报名中',
      disabled: false,
      buttonClass: 'bg-[var(--button-primary-bg)] text-[var(--button-primary-text)]',
    };
  }
...
function resolveRushTaskStatusClass(status) {
  switch (status) {
    case 'pending':
      return 'text-[11px] px-2 h-5 inline-flex items-center rounded-full text-blue-600 bg-blue-500/10 border border-blue-500/20';
    case 'done':
      return 'text-[11px] px-2 h-5 inline-flex items-center rounded-full theme-success-bg theme-success border';
    case 'failed':
