...
const gen = (distance, locations) => {
    ...
    result[0] = `${current[0]}-${current[1]}-${startTime}-${randAccuracy().toFixed(1)}`;
    ...
    while (currentDistance < distance) {
        ...
        for (let j = 0; j < 10; j++) {
            ...
            result[i++] = `${lastRandPos[0]}-${lastRandPos[1]}-${startTime}-${randAccuracy().toFixed(1)}`;
        }
        const distance1 = calculateDistance(lastRandPos, end);
        startTime += distance1 / randInt(1, 5) * 1000;
        result[i++] = `${end[0]}-${end[1]}-${startTime}-${randAccuracy().toFixed(1)}`;
