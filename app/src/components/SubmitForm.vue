        ...
        <el-form ref="formRef" :model="formState" :rules="rules" label-position="left" label-width="auto">
            ...
            <el-row :gutter="16">
                <el-col :xs="24" :sm="12">
                    ...
                    <el-form-item label="跑步里程" prop="distance">
                        <el-input v-model="formState.distance" type="number" :controls="false" style="width: 100%"
                ...
                <el-col :xs="24" :sm="12">
                    ...
                    <el-form-item label="跑步时长" prop="duration">
                        <el-input v-model="formState.duration" type="number" :controls="false" style="width: 100%"
            ...
            <div class="form-info" v-if="formState.distance && formState.duration">
                <span :class="{ 'text-error': !paceLimit }">
                    当前配速: {{ formState.distance === 0 ? "0:00" : formatPace(formState.duration, formState.distance)
                    }}/公里
                </span>
            ...
            <div class="form-footer">
                <el-button plain @click="onRandomFill" :disabled="submitting" class="footer-btn random-btn">
                    随机填充
                </el-button>
                <el-button type="primary" @click="onSubmit" :loading="submitting"
                    :disabled="submitting || !paceLimit || submitting" class="footer-btn submit-btn">
                    提交记录
                </el-button>
            </div>

            <!-- 地图显示 -->
            <RunMapContainer :route="formState.route" :visible="showMap" :distance="formState.distance"
...
const onRandomFill = () => {
    // 生成随机值，保留距离上限（学校上限*1.5），时长不限
    let randomDistance: number, randomDuration: number;
    const schoolDistanceLimit = 5000; // 假设学校设定的上限是5000米
    const maxDistance = schoolDistanceLimit * 1.5; // 上限的1.5倍

    do {
        // 生成1000到maxDistance米之间的随机距离
        randomDistance = Math.floor(Math.random() * (maxDistance - 1000)) + 1000;
        // 生成30分钟以上的随机时间（无上限）
        randomDuration = Math.floor(Math.random() * 120) + 30; // 30到150分钟
